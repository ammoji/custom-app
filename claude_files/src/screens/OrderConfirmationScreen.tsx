...
export default function OrderConfirmationScreen() {
  ...
  const { orderId, total, etaMinutes, shopName } = route.params as {
    orderId: string;
    total: number;
    etaMinutes: number;
    shopName: string;
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top', 'bottom']}>
      <View style={styles.content}>
        ...
        <Text style={styles.subtitle}>We've notified {shopName}</Text>

        <View style={styles.card}>
          <Row label="Order ID" value={orderId} />
          <Row label="ETA" value={`~${etaMinutes} min`} />
          <Row label="Total" value={formatRupees(total)} />
          <Row label="Payment" value="Cash on Delivery" />
        </View>

        <View style={styles.buttons}>
          <Button title="Back to Home" onPress={goHome} fullWidth />
        </View>
      </View>
    </SafeAreaView>
  );
